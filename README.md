...
경제학을 전공하며 효율성과 최적화에 흥미를 느꼈고, 데이터를 다루는 과정에서 도구를 직접 만들어보고 싶다는 갈증이 생겨 프로그래밍을 시작하게 되었습니다. 42Seoul에서 총 24개의 프로젝트를 수행하며 시스템 프로그래밍의 기초부터 실제 서버 구현까지 경험하였고, 문제 해결 능력과 협업 역량을 키웠습니다.

Nginx 유사 서버 구현 프로젝트에서는 서버 구조 설계와 병목 분석을 주도해 5,000건 동시 접속 부하 테스트를 통과했고, 정적 파일 요청 처리 속도를 30% 개선했습니다. Bash 유사 셸 구현 과제에서는 프로세스 병목 문제를 해결해 fork 호출 수를 제한하고 메모리 사용량을 20% 줄였습니다. 또한 Docker 기반 MSA 구조에서 Django, PostgreSQL, Nginx를 활용해 API 응답 속도를 25% 개선한 경험이 있습니다.

문제의 본질을 파악하고 시스템을 안정적으로 설계하는 개발자가 되고자 하며, 변화에 유연하게 대응하면서도 깊이를 놓치지 않는 개발을 지향합니다.
